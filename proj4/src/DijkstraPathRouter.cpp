...
double CDijkstraPathRouter::FindShortestPath(CPathRouter::TVertexID src, CPathRouter::TVertexID dest, std::vector<CPathRouter::TVertexID> &path) noexcept {
    ...
    while (!toVisit.empty()) {
        ...
        toVisit.pop_front();
        // std::cout << "Cur Vertex ID: " << currVertex -> m_id << std::endl;
        visited[currVertex->m_id] = true;

        // sort m_adjacents from min to max
        auto sortAdjacenciesMin = [](auto &pair1, auto &pair2) {
            return pair1.second < pair2.second;
        };

        // std::cout << "sorted: " << std::endl;
        std::sort(currVertex->m_adjacents.begin(), currVertex->m_adjacents.end(), sortAdjacenciesMin);
        // for(int k = 0; k < currVertex->m_adjacents.size(); k++) {
        //     std::cout << currVertex->m_adjacents[k].first->m_id << std::endl;
        // }
        ...
        for (int j = 0; j < currVertex->m_adjacents.size(); j++) {
            if (!visited[currVertex->m_adjacents[j].first->m_id]) {
                ...
                if (tmpDist < distances[currVertex->m_adjacents[j].first->m_id]) {
                    ...
                    // std::cout << "  prevVertex: " << prevVertex[currVertex->m_adjacents[j].first->m_id]->m_id << std::endl;
                }
