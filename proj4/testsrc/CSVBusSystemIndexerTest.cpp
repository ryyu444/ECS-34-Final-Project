...
TEST(CSVBusSystemIndexer, RouteTest){
...

    {
        ...
        EXPECT_EQ(BusSystemIndexer.StopCount(),3);
        ...
        EXPECT_EQ(Route2Index->Name(),"B");
        EXPECT_EQ(Route2Index->StopCount(),3);
        EXPECT_EQ(Route2Index->GetStopID(0),3);
        EXPECT_EQ(Route2Index->GetStopID(1),2);
        EXPECT_EQ(Route2Index->GetStopID(2),3);
        auto Route3Index = BusSystemIndexer.SortedRouteByIndex(2);
        ASSERT_TRUE(bool(Route3Index));
        EXPECT_EQ(Route3Index->Name(),"K");
        EXPECT_EQ(Route3Index->StopCount(),2);
        EXPECT_EQ(Route3Index->GetStopID(0),2);
        EXPECT_EQ(Route3Index->GetStopID(1),1);
        std::unordered_set< std::shared_ptr<CBusSystem::SRoute> > Routes;
        EXPECT_TRUE(BusSystemIndexer.RoutesByNodeIDs(101,102,Routes));
        EXPECT_EQ(Routes.size(),1);
        EXPECT_TRUE(Routes.find(Route1Index) != Routes.end());
        EXPECT_TRUE(Routes.find(Route2Index) == Routes.end());
        EXPECT_TRUE(Routes.find(Route3Index) == Routes.end());
    }
